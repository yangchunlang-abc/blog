import{_ as e,o as n,c as i,e as s}from"./app-abc032b7.js";const d={},l=s(`<blockquote><p>前言</p></blockquote><p>JWT（JSON Web Token）是一种用于在网络应用之间传递声明的方法，它是一种基于 JSON 的开放标准（RFC 7519），用于在网络上安全地传输信息。</p><p>JWT 令牌通常被用于身份验证和授权，例如当用户登录时，服务器可以生成一个 JWT 令牌，并将其发送回客户端。客户端在接下来的请求中将该令牌作为身份验证凭证发送给服务器，服务器可以验证令牌的有效性，并根据令牌中的声明授权用户访问相应的资源。</p><h3 id="下载jsonwebtoken" tabindex="-1"><a class="header-anchor" href="#下载jsonwebtoken" aria-hidden="true">#</a> 下载jsonwebtoken</h3><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>npm install jsonwebtoken
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>只有令牌验证通过了才能进行next(),或者没有令牌也能通过</p><p>具体来说，该中间件函数会检查请求体中是否包含 token 字段。如果包含，就将该字段的值作为令牌，调用 jwt.verifyToken 函数进行验证。如果验证通过，就调用 next() 方法继续处理下一个中间件或路由处理函数；如果验证不通过，就返回一个 JSON 响应 { status: 3000 }，表示验证失败。</p><p>如果请求体中不包含 token 字段，就直接调用 next() 方法继续处理下一个中间件或路由处理函数，即跳过令牌验证步骤。</p><p>在前端每次调用请求的时候在req.body添加taken字段，就可以实现验证JWT!!!!</p><p>但是一般都是令牌是在header里的，这里放body也因为这是一个app而不是web应用..</p>`,19),a=[l];function r(v,t){return n(),i("div",null,a)}const u=e(d,[["render",r],["__file","7.shezhiJWTlingpai.html.vue"]]);export{u as default};
